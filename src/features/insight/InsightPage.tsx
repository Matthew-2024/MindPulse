  ...
  return (
    <div className="page-content insight-page">
      ...
      <section className="insight-overview">
        ...
        <div className="insight-overview-item">
          <ShieldAlert size={17} />
          <div><span>接下来怎么做</span><strong>{modeLabel(decision.risk.mode)}</strong><small>{decision.risk.reason}</small></div>
        </div>
      </section>

      <section className="section-block contribution-section">
        <div className="section-heading"><div><span className="eyebrow">各项记录的影响</span><h2>这次分数由什么组成</h2></div><span className="section-meta">没填的项目不会被当成 0</span></div>
        <div className="contribution-list">
          {decision.score.contributions.map((item) => (
            <div className="contribution-row" key={item.signal}>
              ...
              <span className="contribution-reason">{item.reason}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="section-block counterfactual-section">
        ...
        <div className="simulation-result">
          <div><span>模拟分数</span><strong>{simulatedDecision.score.total}</strong></div>
