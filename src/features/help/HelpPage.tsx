...
import { appendResourceOperation, LOCAL_RESOURCE_OPERATION_TENANT } from "../../domain/resource-operation-store";
...
export function HelpPage() {
  ...
  const sourceTypes = Array.from(new Set(Object.values(decision.baseline.dataCompleteness.sources || {}).filter(Boolean)));
  ...
  function recordResourceAction(resourceId: string, kind: ResourceOperationKind) {
    if (typeof window === "undefined") return;
    try {
      appendResourceOperation(window.localStorage, { tenantId: LOCAL_RESOURCE_OPERATION_TENANT, resourceId, kind });
    } catch {
      // Local metadata telemetry must never prevent a user-initiated support action.
    }
  }
    ...
    <div className="page-content help-page">
      <div className="page-intro">
        <div>
          ...
          <p>{highRisk ? "当前判断会先暂停普通自助行动，只保留你主动选择的求助路径。系统不会自动上报。" : "你可以提前准备一句话，不需要等到把事情解释得很完整。"}</p>
        </div>
        {!highRisk ? <Link className="icon-link" to="/settings" aria-label="配置求助资源" title="配置求助资源"><Settings2 size={19} /></Link> : null}
      </div>

      {highRisk ? <SafetyGate decision={decision} resources={state.helpResources} onResourceAction={recordResourceAction} /> : (
      ...
      <HelpComposer
        ...
        decisionId={decision.trace.decisionId}
        sourceTypes={sourceTypes}
        highRisk={highRisk}
        onLog={recordHelpEvent}
        onResourceCopy={() => recordResourceAction("offline-support-fallback", "copy-requested")}
      />

      <section className="help-boundary">
        <div><ShieldCheck size={17} /><strong>你的选择保持在你手里</strong></div>
        <p>求助资源保存在本机；只有你主动复制或发送时，内容才会离开本机。</p>
