...
function allowedActionList(decision: MindPulseDecision) {
  if (decision.risk.mode === "ask") return ["checkin"];
  return Array.from(new Set(decision.trace.allowedActions));
}

function blockedActionList(decision: MindPulseDecision) {
  if (isHighRisk(decision.risk)) return ["普通行动已暂停"];
  if (decision.risk.mode === "ask") return ["普通行动暂不建议"];
  return decision.trace.blockedActions.length ? decision.trace.blockedActions : ["无"];
}
  ...
  return (
    <section className="trace-panel">
      ...
      <div className="trace-columns">
        ...
        <div>
          ...
          <div className="action-tags action-tags-allow">
            {allowedActionList(decision).map((item) => <span key={item}>{actionLabel(item)}</span>)}
          </div>
        </div>
        <div>
          <h3><Ban size={15} /> 暂时不建议</h3>
          <div className="action-tags action-tags-block">
            {blockedActionList(decision).map((item) => <span key={item}>{item === "普通行动已暂停" || item === "普通行动暂不建议" ? item : actionLabel(item)}</span>)}
          </div>
        </div>
      </div>
      ...
      <div className="trace-footer">
        ...
        <span>规则版本：{trace.policyVersion}</span>
