...
async function requireSession(req: Request) {
  ...
  if (!device) throw new Error("设备已撤销或不可用");
  const now = new Date().toISOString();
  await supabase.from("sessions").update({ last_seen_at: now }).eq("id", session.id);
  await supabase
    .from("devices")
...
async function requestEmailCode(req: Request, body: JsonRecord) {
  ...
  if (resendKey) {
    ...
    return json(req, { error: "Email delivery is not configured" }, 503);
  }
