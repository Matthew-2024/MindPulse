...
- 邮箱注册 / 登录：demo 版固定验证码 `246810`，验证结果只保存到本机；邮箱为空或格式不完整时不会通过。
- 记录：情绪、睡眠、步数、连接感、精力、文字记录；每次保存都会追加为一条即时记录。
- 日报 / 周报：今日时间线、日报均分、主要情绪、7 天周报和 Safety Gate 次数。
...
- 与 Web demo 版同步固定验证码 `246810`、本地演示同步状态、`vault_id` 展示、即时记录追加、日报和周报；开启演示同步前必须先完成邮箱验证。

## 当前限制

- 数据目前保存在本机 `UserDefaults`，适合比赛原型和单机演示。
- 三个人跨设备共享数据需要后续接入后端或 CloudKit。
- demo 邮箱验证码不请求 Supabase/Resend；正式试用前需要替换为真实邮件服务或平台身份方案。
