...
const requiredFiles = [
  ...
  "tests/profile-strategy-smoke.js",
  "src/rules/browser-engine.js",
  "tools/build-browser-engine.js",
  "src/state/store.js",
  ...
  "supabase/functions/request-email-code/index.ts",
  "supabase/functions/verify-email-code/index.ts",
  "supabase/functions/sync-vault/index.ts",
  "supabase/functions/get-vault-copy/index.ts",
  "supabase/functions/clear-vault-copy/index.ts",
  "tools/supabase-remote-deploy-notes.md",
...
const requiredText = [
  ["README.md", ["不做心理诊断", "Safety Gate", "合成演示数据", "日报", "周报", "IndexedDB", "vault_id", "Supabase", "encrypted_items", "npm.cmd run serve:web", "docs/review-evidence/", "npm.cmd run preflight", "npm.cmd run audit:docx"]],
  ["docs/自我审查项目.md", ["A07 | 是否有真实用户研究结果 | 待执行", "A08 | 是否有专业审核意见 | 待执行", "20 条规则测试"]],
  ["docs/数据使用说明.md", ["合成演示数据", "公开数据集验证", "真实用户研究", "email_verifications", "encrypted_items", "明文验证码"]],
  ["docs/Supabase云端同步方案.md", ["request-email-code", "verify-email-code", "sync-vault", "get-vault-copy", "clear-vault-copy", "code_hash", "HttpOnly; Secure", "encrypted_items.ciphertext", "不要求部署本地 Supabase", "npx supabase db push"]],
  ...
  ["docs/当前测试与自查报告.md", ["20 条规则/个性化用例通过", "npm.cmd run preflight", "npm.cmd run audit:self", "npm.cmd run audit:docx", "npm.cmd run demo:smoke", "日报/周报", "dailyReport", "weeklyReport", "加密同步", "ciphertext", "docs/review-evidence/", "A07 真实用户研究结果 | 待执行"]],
  ["docs/提交前自查矩阵.md", ["npm.cmd run preflight", "npm run audit:docx", "即时记录", "今日/周报", "Supabase 云端同步模板", "A07 真实用户研究 | 待执行", "A08 专业审核 | 待执行", "不能写成已完成", "合成演示数据用于演示和测试规则链路"]],
  ...
  ["tools/build-browser-engine.js", ["src/rules/signals.js", "src/rules/recovery-score.js", "src/rules/risk-assessment.js", "MindPulseRules"]],
  ["src/state/store.js", ["MindPulseStateStore", "snapshot", "apply"]],
  ["src/security/risk-gate.js", ["MindPulseRiskGate", "blocksAction", "canShowSelfCheck"]],
  ["src/selectors/score-selectors.js", ["MindPulseScoreSelectors", "beforeAfter", "averageDelta", "validationReport"]],
  ["tests/ui-smoke.js", ["mindpulse-records.json", "exported.risk", "exported.dailyReport", "exported.weeklyReport", "syncPayload.ciphertext", "plaintext", "clearDialogMessage", "afterClear.vaultRecord === null", "afterClear.sensitiveKeys.length === 0"]],
  ["tests/demo-flow-smoke.js", ["Demo flow smoke passed", "今日记录", "周报", "vault_id", "encrypted_items", "很需要支持", "mindpulse-records.json", "自动化规则测试：20 / 20"]],
  ["supabase/migrations/20260618_mindpulse_secure_sync.sql", ["accounts", "email_verifications", "code_hash", "devices", "sessions", "vaults", "encrypted_items", "ciphertext"]],
  ["supabase/config.toml", ["verify_jwt = false", "request-email-code", "sync-vault", "get-vault-copy"]],
  ["supabase/.env.example", ["SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY", "CODE_HASH_SECRET", "ALLOWED_ORIGINS"]],
  ["tools/supabase-remote-deploy-notes.md", ["不需要启动本地 Supabase", "npx supabase link", "npx supabase db push", "--no-verify-jwt"]],
  ["supabase/functions/request-email-code/index.ts", ["randomCode", "codeHash", "resendAfterSeconds", "email_verifications"]],
  ["supabase/functions/verify-email-code/index.ts", ["assertCode", "attempts", "sessionCookie", "vaultId"]],
  ["supabase/functions/sync-vault/index.ts", ["ciphertext", "nonce", "plaintext", "records", "encrypted_items", "keyVersion", "kdf", "iterations"]],
  ["supabase/functions/get-vault-copy/index.ts", ["encrypted_items", "ciphertext", "nonce", "iterations", "requireSession"]],
