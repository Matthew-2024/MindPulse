...
| 层 | 用途 |
|---|---|
| `localStorage` | 兼容当前 Web 原型包、保存账号状态、同步开关和非敏感 UI 状态 |
...
npx supabase functions deploy resource-admin --no-verify-jwt --use-api
```

## 7.1 Resource Operations Tenant Boundary

`supabase/migrations/20260804_resource_operations_tenant_boundary.sql` adds a server-side resource tenant, explicit admin membership, public support-resource, and resource-only operation metadata model. It contains no student vault, record, note, risk, email, phone, recipient, or contact-import field in the operations table.

The `resource-admin` Function requires an existing authenticated session and active `resource-admin` or `resource-owner` membership for the requested tenant. It can publish a resource as unverified, verify it with a review expiry, invalidate it with a reason, list tenant-scoped resources, and return 90-day resource-health aggregates only after five qualifying copy/link operations.

Before deployment, a responsible operator must create the tenant and administrator membership with verified organization authority. This template is not a deployed role system, does not grant anyone administrator status, and does not let the current React local-admin page contact the service automatically.
