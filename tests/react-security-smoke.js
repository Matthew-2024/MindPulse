...
assert(await page.locator('[data-testid="offline-support-fallback"]').count() === 1, "high-risk help must expose an offline fallback action");
assert((await page.locator('[data-testid="offline-support-fallback"]').innerText()).includes("急救号码"), "offline fallback should remain actionable without configured resources");
...
await page.waitForURL(/\/help$/);
assert(await page.locator(".trusted-circle-page").count() === 0, "high-risk state must redirect Trusted Circle to the help-only route");
...
const reassessedVault = await page.evaluate(async () => {
  const vaultId = localStorage.getItem("mindpulseReactVaultId");
  return await new Promise((resolve) => {
    const request = indexedDB.open("mindpulse-local-vault", 2);
    request.onsuccess = () => {
...
assert(reassessedVault?.records?.length >= 2, "reassessment should keep the original high-risk record");
assert(reassessedVault.records.some((record) => String(record.note || "").includes("从这个世界上消失")), "original high-risk note should remain local after reassessment");
assert(reassessedVault.safetyHold?.active === false && reassessedVault.safetyHold?.releasedAt, "safe reassessment should release the persisted safety hold with a cutoff");
assert((reassessedVault.safetyEvents || []).some((event) => event.kind === "triggered"), "safety lifecycle should record the original trigger");
assert((reassessedVault.safetyEvents || []).some((event) => event.kind === "reassessment-opened"), "safety lifecycle should record the reassessment request");
assert((reassessedVault.safetyEvents || []).some((event) => event.kind === "released"), "safety lifecycle should record the release criteria outcome");
