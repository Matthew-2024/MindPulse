...
assert(body.includes("所在地区") && body.includes("校内支持链接") && body.includes("最近核验日期"), "settings should expose warm-handoff verification fields");
...
assert(body.includes("分享前确认") && body.includes("不会包含") && body.includes("只有你主动复制或发送时"), "help bridge should expose a minimum-disclosure preview");
...
for (const viewport of [{ width: 375, height: 667 }, { width: 430, height: 932 }, { width: 1024, height: 768 }]) {
  ...
  if (viewport.width <= 430) {
    assert(layout.navPosition === "relative", `${viewport.width}px should keep the bottom tab bar in the shell layout`);
