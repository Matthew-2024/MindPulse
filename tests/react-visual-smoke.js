...
for (const viewport of [{ width: 375, height: 667 }, { width: 430, height: 932 }, { width: 1024, height: 768 }]) {
  ...
  assert((body.includes("记录指数") && body.includes("不是健康分数")) || body.includes("当前参考指数"), `${name} missing score boundary`);
...
for (const route of ["/checkin", "/insight", "/companion", "/help", "/reports", "/settings", "/rules", "/bottle"]) {
  const label = `route-${route.slice(1) || "home"}-487`;
  const { context, page } = await openContext({ width: 487, height: 872 }, label);
  ...
  const introLayout = await page.evaluate(() => {
    ...
    return !backLinkRect || !eyebrowRect || eyebrowRect.top >= backLinkRect.bottom + 6;
  });
  assert(introLayout, `${label} should keep the back link and page eyebrow on separate rows`);
